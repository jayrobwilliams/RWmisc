...
```{r, results = 'hide'}
library(brms)
fit1 <- brm(mpg ~ cyl + disp + hp, data = mtcars, family = gaussian())
```
...
You can pass any standard `texreg` arguments not already defined in `mcmcreg`, such as the `custom.coef.map` argument that I use here to rename and reorder the parameters.
...
`mcmcreg()` will automatically include any random effects in tables as well.
...
If you wish to include other parameters from more complex `brms` models, simply identify the parameter family names by inspecting `brmsfit$fit@model_pars`.
