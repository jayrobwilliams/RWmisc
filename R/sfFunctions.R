...
## these functions use the sf library to perform a variety of spatial operations,
## primarily related to automatically projecting spatial data to the appropriate
## UTM zone.
...
projectUTM <- function(sf.object) {
  ...
  if (attr(st_geometry(sf.object), 'class')[1] == 'sfc_MULTIPOLYGON') {
...
point_polydist <- function(poly, point, max = T) {
